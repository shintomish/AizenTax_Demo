...
class BillDataController extends Controller
{
    ...
    public function index()
    {
        ...
        Log::info('billdata index START $user->name = ' . print_r($user->name ,true));
        ...
        Log::debug('billdata index customer_id  = ' . print_r($customer_id ,true));
        ...
        $jsonfile = storage_path() . "/tmp/billdata_info_status_". $customer_id. ".json";
            // Log::debug('billdata index $jsonfile  = ' . print_r($jsonfile ,true));

        $keyword2  = null;
        $common_no = '06_2';
        $compacts = compact('keyword2','nowyear','common_no','userid','billdatas', 'customer_findrec','customer_id','jsonfile' );

        Log::info('billdata index END');

        return view( 'billdata.index', $compacts );
    }
    ...
    {
        Log::info('billdata serch_custom START');
    ...
    {
        Log::info('billdata show_up01 インボイス START');
        ...
        Log::info('billdata show_up01 インボイス END');
    ...
    {
        Log::info('billdata postUpload_info  START');
        ...
        Log::info('billdata postUpload_info $compacts[customer_id]  = ' . print_r($compacts['customer_id'] ,true));
        ...
        Log::info('billdata postUpload_info  END');
    ...
    {
        Log::info('billdata postUpload  START');
        ...
        if (file_exists($jsonUrl)) {
            ...
            if(empty($obj)){
                ...
                Log::info('billdata postUpload empty');
            } else {
                // $obj = $obj["res"]["info"];
                $obj[0] = $this->postUpload_info($customer_id);
                Log::info('billdata postUpload not empty');
            }
            ...
            //     $status = $val["status"];
            // }
            // Log::info('billdata postUpload  jsonUrl OK');
        } else {
            // echo "データがありません";
            // Log::info('billdata postUpload  jsonUrl NG');
        ...
        if ($totalSize && $totalSize > $maxtatalsize)
        {
            ...
            Log::info('billdata postUpload  failesize to big ');
            // Statusを変える
            $status = false;
            $this->json_put_status($status,$customer_id);
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            if ($file->checkChunk()) {
                ...
                Log::info('billdata postUpload HTTP/1.1 200 Ok ');
            } else {
                ...
                Log::info('billdata postUpload HTTP/1.1 204 No Content ');
                return ;
            }
        } else {
            if ($file->validateChunk()) {
                // Log::info('billdata postUpload validateChunk ok ');
                $file->saveChunk();
            } else {
                ...
                Log::debug('billdata postUpload HTTP/1.1 400 Bad Request ');
                return ;

                // // strage/tmp
                // $file->deleteChunks();
                // Log::debug('billdata postUpload HTTP/1.1 400 Bad Request ');
                // Log::debug('billdata postUpload_info  validateChunk not   $uploadFile[name]  = ' . print_r($uploadFile['name'] ,true));
        ...
        Log::info('billdata postUpload  $fileName = ' . print_r($fileName,true));
        if ($file->validateFile() && $file->save($storage_path))
        {
            ...
            Log::info('billdata postUpload  END');
            ...
            Log::info('billdata postUpload  This is not a final chunk, continue to upload ');
        }
    ...
    {
        Log::info('billdata json_put_status  START');
        ...
        Log::info('billdata json_put_status  END');
    }
    ...
    {
        Log::info('billdata json_put_info_set  START');
        ...
        Log::info('billdata json_put_info_set  END');
    }
    ...
    {
        Log::info('billdata json_get_info  START');

        $jsonfile = storage_path() . "/tmp/billdata_info_". $customer_id. ".json";

        // Log::debug('billdata json_get_info  jsonfile = ' . print_r($jsonfile,true));
        ...
        if (file_exists($jsonUrl)) {
            ...
            if(empty($obj)){
                $obj[0] = $this->postUpload_info($customer_id);
                Log::info('billdata json_get_info empty');
            } else {
                $obj = $obj["res"]["info"];
                Log::info('billdata json_get_info not empty');
            }
            ...
            $check_flg = 1;
            // Log::info('billdata json_get_info  OK');
        } else {
            echo "データがありません";
            Log::info('billdata json_get_info  NG');
        }
        $compacts = compact( 'u_id','o_id', 'customer_id', 'foldername','business_name','folderpath','dateNew' );

        Log::info('billdata json_get_info  END');
        return  $compacts;
    }
